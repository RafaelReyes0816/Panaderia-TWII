...
@section('content')
    ...
    <form action="{{ route('compras.store') }}" method="POST" class="card p-4">
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" id="precio-unitario" class="form-control" min="0">
        </div>
...
function renderResumen() {
    ...
    tbody.innerHTML = '';
    let total = 0;
    productosCompra.forEach((item, idx) => {
...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('add-producto').addEventListener('click', function() {
        ...
        if (!productoId || !productosData[productoId] || precioUnitario <= 0) {
